...
class Test(IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.repo = AsyncMock(IQueryFlowAnswerRepo)

        if self.test_type == "unit":
            self.service = QueryFlowAnswerRepoHTTPServer().set_repo(self.repo)
            self.service.set_http_port(http_port)
            await self.service.listen()

        # Initialise the client(s) here
        self.client = QueryFlowAnswerRepoHttpClient()
        (
    ...
    async def test_get_query_flow_answers(self) -> None:
        ...
        patch.object(
            self.repo,
            "get_query_flow_answers",
            AsyncMock(return_value=Response.ok(answers)),
        ).start()

        # Act
        response = await self.client.get_query_flow_answers(
            GetQueryFlowAnswersRequest()
        )

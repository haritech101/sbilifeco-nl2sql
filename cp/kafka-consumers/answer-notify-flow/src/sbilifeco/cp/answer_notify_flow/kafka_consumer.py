...
from sbilifeco.boundaries.answer_notify_flow import AbstractAnswerNotifyFlow
from sbilifeco.cp.common.kafka.consumer import PubsubConsumer
from sbilifeco.cp.answer_notify_flow.paths import Paths


class AnswerNotifyTriggerConsumer(PubsubConsumer):
    def __init__(self):
        super().__init__()
        self.set_consumer_name("answer-notify-trigger-consumer")
        self.add_subscription(Paths.BASE.replace("/", ".").lstrip("."))
        self.flow: AbstractAnswerNotifyFlow

    def set_flow(self, flow: AbstractAnswerNotifyFlow) -> AnswerNotifyTriggerConsumer:
        self.flow = flow
        return self

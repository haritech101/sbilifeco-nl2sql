...
from sbilifeco.boundaries.query_flow import IQueryFlowAnswerRepo, QueryFlowAnswer
...
class Test(IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.repo = AsyncMock(spec=IQueryFlowAnswerRepo)
        self.presenter = AsyncMock(spec=IAnswerPresenter)

        if self.test_type == "unit":
            self.service = AnswerNotifyFlow().set_non_sql_answer_repo(self.repo)
    ...
    async def test_fetch_and_notify(self) -> None:
        ...
        fn_fetch = patch.object(
            self.repo, "get_query_flow_answers", return_value=Response.ok(answers)
        ).start()
